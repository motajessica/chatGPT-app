...
//Try to change the API KEY for one that works. 
...
const App = () => {
  ...
  const sendMessageToChatGPT = async (message) => {
    try {
      const response = await fetch('https://te-wao-nui-story.openai.azure.com/openai/deployments/te-wao-nui-storyline/chat/completions?api-version=2023-03-15-preview', {
        method: 'POST',
        headers: {
    ...
    <div>
      ...
      <div>
        {CHARACTERS.map((character) => (
          <button ref={buttonRef} key={character.name} onClick={() => handleCharacterSelection(character)}>
            {character.name}
          </button>
        ))}
      </div>
